...
def fetch_title():
    ...
    if title_tag:
        sb(f"Page Title: {title_tag.text.strip()}")
    else:
        sb("No title tag found")
...
def fetch_links():
    ...
    for link in soup.findAll('a', href=True):
        ...
        if href.startswith(('http', 'https')):
            # Check if the link is external (doesn't contain the base domain)
            if not href.startswith('http://localhost'):
                external_links.append(href)

    # Display external links in the content field (Listbox)
    if external_links:
        _images.set(tuple(external_links))  # Use _images variable for external links
        sb(f"Found {len(external_links)} external links")
    else:
